...
name: marketing-plan
description: |
  When the user needs a comprehensive marketing plan for a client, a company they advise, or their own product. Also use when the user mentions "marketing plan," "growth plan," "GTM plan," "go-to-market plan," "AARRR plan," or "fractional CMO plan." Generates a 13-section plan structured by AARRR (Acquisition, Activation, Retention, Referral, Revenue), customized to the client's budget, team, and stage, cross-referenced with a 139-idea library and a 17-section current-state audit rubric. Outputs a Notion-paste-ready markdown document. 日本語トリガー例: マーケティング計画を作って／マーケ戦略の12ヶ月ロードマップ／GTM計画／集客の全体設計／販促計画を立てて。経営理論のフレームワーク分析（SWOT・4P・財務指標・診断士の事例I〜IV）には sme-consultant を使う（本スキルは実行計画の作成が目的）。価格・プラン設計のみなら pricing、申込率改善のみなら cro を使う。
---
