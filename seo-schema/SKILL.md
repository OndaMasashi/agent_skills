...
name: seo-schema
description: |
  Detect, validate, and generate Schema.org structured data. JSON-LD format preferred. Use when user says 'schema', 'structured data', 'rich results', 'JSON-LD', or 'markup'. Includes the current active/deprecated schema type list (as of June 2026) and Google's supported rich result types. 日本語トリガー例: 構造化データを作って／JSON-LDを生成して／リッチリザルトに対応したい／schema.orgのマークアップを検証して／パンくずやFAQの構造化データ／この型はまだ有効か。AI検索（AI Overviews・ChatGPT・Perplexity）向けの最適化は seo-geo を使う（本スキルは構造化データの検出・検証・生成に特化）。
user-invocable: true
argument-hint: "[url]"
license: MIT
metadata:
