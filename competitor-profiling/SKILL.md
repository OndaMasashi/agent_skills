...
description: |
  When the user wants to research, profile, or analyze competitors from their URLs. Also use when the user mentions 'competitor profile,' 'competitor research,' 'competitor analysis,' 'competitive intelligence,' 'competitor landscape,' or 'competitive audit.' Input is a list of competitor URLs. Output is structured, side-by-side comparable competitor profile markdown files, with raw data saved by date so profiles can be diffed over time. 日本語トリガー例: 競合を調べて／競合分析して／競合サイトを比較して／競合の料金体系を調べて／競合の訴求を整理して／競合リサーチをまとめて。技術・市場テーマの一般調査は deep-research を使う（本スキルは競合URLから構造化プロファイルを作るのが目的）。Firecrawl / DataForSEO MCP は未導入のためSEO指標は取得できない（SKILL.md 冒頭の注記を参照）。
metadata:
  version: 2.0.0
---
...
> **ローカル環境での代替手段（このリポジトリ固有の注記）**
>
> 本スキルは Firecrawl MCP と DataForSEO MCP を前提に書かれているが、**どちらも未導入**。
> 該当ツールを呼ぶ前に接続状況を確認し、無ければ以下で代替すること。
>
> - **Phase 1（サイト取得）**: `firecrawl_map` / `firecrawl_scrape` → **WebFetch で代替可**。
>   ページ一覧の自動発見はできないため、トップ・料金・機能・会社概要などを個別 URL で辿る。
> - **Phase 2（SEO・市場データ）**: `dataforseo_*` 系（流入キーワード・ドメインランク・被リンク・競合ドメイン検出）
>   → **代替不可**。当該節はスキップし、プロファイル内で「未取得」と明示する（推測値で埋めない）。
> - 自社サイト（reverve-consulting.com）の実測値のみ reverve-hp MCP から取得できる。競合サイトには使えない。
>
> MCP を導入すれば無改造でフル機能になる。
