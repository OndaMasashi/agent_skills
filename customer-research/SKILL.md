...
description: |
  When the user wants to conduct, analyze, or synthesize customer research. Use when the user mentions 'customer research,' 'ICP research,' 'customer interviews,' 'survey analysis,' 'voice of customer,' 'VOC,' 'customer personas,' 'jobs to be done,' 'JTBD,' 'review mining,' or 'why customers churn/convert/buy.' Covers both analyzing existing assets (transcripts, surveys, support tickets, win/loss, NPS) and gathering new research from online sources (Reddit, G2, forums). 日本語トリガー例: 顧客インタビューを分析して／アンケート結果をまとめて／顧客の生の声を抽出して／ペルソナを作って／JTBDで整理して／解約理由を分析して／口コミを分析して。技術文献や市場全体の外部調査は deep-research、経営理論に基づく診断は sme-consultant を使う（本スキルは顧客の発言・行動データから示唆を出すのが目的）。
metadata:
  version: 2.0.1
---
